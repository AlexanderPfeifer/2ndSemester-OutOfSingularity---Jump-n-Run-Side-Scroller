...
public class EnemyScript : MonoBehaviour
{
    ...
    /// </summary>
    public int firstPatrolSpot = -10;

    public int secondPatrolSpot = 10;
    /// <summary>
    ...
    private void Start()
    {
        ...
        randomSpot = Random.Range(firstPatrolSpot, secondPatrolSpot);
        enemyAttack = AudioManager.instance.CreateInstance(FMODEvents.instance.enemyAttack);
        randomPosition = new Vector2(randomSpot, 0); 
    }
    ...
    private void Update()
    {
        if(canPatrol)
        {
            ...
            {
                Vector2.MoveTowards(transform.position, rb.position, -speed * Time.deltaTime);
            }

            if (Vector2.Distance(transform.position, randomPosition) < Mathf.Epsilon)
            {
                if (waitTime <= 0)
                {
                    randomSpot = Random.Range(firstPatrolSpot, secondPatrolSpot);
